...
First we combine all test data in a dataframe which contain average spike and feedback.
```{r eval=TRUE, echo=FALSE}
test=list()
for(i in 1:2){
  test[[i]]=readRDS(paste('~/Downloads/test-2/test',i,'.rds',sep=''))
}
```
...
# Iterate over each session
for (i.s in 1:length(test)) {
  n_trials <- length(test[[i.s]]$feedback_type)
  
  # Create a data frame for the current session
  df_test <- data.frame(test = rep(i.s, n_trials),
                           Trial = 1:n_trials,
                           Mouse = rep(session[[i.s]]$mouse, n_trials),
                           averagespike = numeric(n_trials),
                           stringsAsFactors = FALSE)
  ...
  for (i.t in 1:n_trials) {
    ...
    total_spikes <- apply(spks_trial, 1, sum)
    avg_spikes <- mean(total_spikes)
    df_test$averagespike[i.t] <- avg_spikes
  }
...
for( i in length(test)){
  n.trial=length(test[[i]]$feedback_type)
  n.area=length(unique(test[[i]]$brain_area))
# Alternatively, you can extract these information in the meta that we created before.

# We will create a data frame that contain the average spike counts for each area, feedback type,  the two contrasts, and the trial id
 trial.summary = matrix(nrow=n.trial,ncol=1+2)
 for(i.t in 1:n.trial){
   trial.summary[i.t,]=c(test[[i]]$feedback_type[i.t],
                        test[[i]]$contrast_left[i.t],
                        test[[i]]$contrast_right[i.t])
  }

  colnames(trial.summary)=c('feedback', 'left_contr','right_contr')
  trial.summary<- as.data.frame(trial.summary)
  trialsummary1[[i]]=trial.summary

}

trailscombined <- bind_rows(trialsummary1)
combine_df1 <- cbind(df1, trailscombined)
head(combine_df1)
```
### predict model for test data
After evaluating the model's performance on the test data, we found that the misclassification rate was approximately 0.425. This indicates that around 42.5% of the observations in the test dataset were misclassified by the prediction model. Therefore, the model's accuracy in predicting the feedback types is relatively low.
The result showed that the average spike count had a coefficient of 0.2027. This suggests that an increase in the average spike count is associated with a higher likelihood of the feedback being positive.In summary, our research revealed that accurately predicting the outcome of decision-making trials based on neural activity data is a complex task. Despite our efforts to develop a predictive model using logistic regression, we encountered challenges in achieving high accuracy.
...
# Set a seed for reproducibility
set.seed(123)
data1<- combine_df1$feedback
# Create the data partition
split<- sample.split(data1,SplitRatio=0.80)
# Split the data
train_data <- subset(combine_df1,split == TRUE)
test_data <- subset(combine_df1,split == FALSE)
logi <- glm(feedback ~ averagespike, data = train_data)
logi
pred <- predict(logi, newdata = test_data, type = 'response') 
pred_class <- ifelse(pred > 0.5, '-1', '1')              
# Confusion matrix from logistic
...
Furthermore, we constructed a logistic regression model to predict the feedback types using the available variables. However, the model's performance, as demonstrated by the confusion matrix, reveals a relatively high misclassification rate of 0.500. This indicates that the model's accuracy in predicting the feedback types is limited.we still have a challenge to increase the accuracy. It suggests that there may be other factors influencing the decision-making process that are not accounted for by the selected variables.

In conclusion, our project has provided valuable insights into the data features, neural activity patterns, and prediction of feedback types in mice during a decision-making task. While we have identified some interesting trends, it is important to acknowledge the limitations of low accuracy in the final model. Therefore,Future research can further explore and enhance our findings, ultimately contributing to a more comprehensive understanding of the factors that influence decision making in mice.
